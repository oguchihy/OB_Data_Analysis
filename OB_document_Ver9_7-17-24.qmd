...
#write.xlsx(combined_data_orig_base, "orig_files/OB Delivery Data Base Set, combined 2022-2024Q1.xlsx")

# unique_pts <- combined_data_orig_base %>% 
#   distinct(mr_number, .keep_all = TRUE)
# 
# unique_count <- nrow(unique_pts)
# unique_percent <- round(nrow(unique_pts)/nrow(combined_data_orig_base)*100,1)

# Summarize the count of rows per group
group_counts <- combined_data_orig_base %>%
  group_by(mr_number) %>%
  summarise(n = n()) %>% 

# Count the number of groups where the count is greater than 1 and greater than 2
...
- **```r group_counts$just_3```**: three deliveries

- **```r group_counts$gt_3```**:  greater than three deliveries
