...
file_paths <- c("orig_files/CSVS Delivery Log 2022zcd.xlsx", 
                "orig_files/CSVS Delivery Log 2023zcd.xlsx", 
                "orig_files/CSVS Delivery Log 2024 Qtr 1zcd.xlsx")  # Adjusted to reflect different files
...
combined_data_orig_base <- bind_rows(data_list)

# Clean column names
# Clean column names
combined_data_orig_base <- combined_data_orig_base %>%
  janitor::clean_names()

# Step 1: Identify unique deliveries
# Adjust columns below as necessary to uniquely identify each delivery
unique_deliveries <- combined_data_orig_base %>%
  distinct(mr_number, adm_date, adm_time, .keep_all = TRUE)

# Step 2: Count the number of deliveries per mr_number, adjusting for twin gestations
delivery_counts <- unique_deliveries %>%
  group_by(mr_number) %>%
  summarise(
    deliveries = n(),
    twin_gestation = any(baby_sq > 1)  # identify if there is a twin gestation
  ) %>%
  ungroup()

# Step 3: Adjust the count for twin gestations
delivery_counts <- delivery_counts %>%
  mutate(deliveries = ifelse(twin_gestation, deliveries / 2, deliveries))

# Step 4: Summarize the counts based on the occurrences
group_counts <- delivery_counts %>%
  summarise(
    just_1 = sum(deliveries == 1),
    just_2 = sum(deliveries == 2),
    just_3 = sum(deliveries == 3),
    gt_3 = sum(deliveries >= 3)
  )

just_1_pcnt <- round(group_counts$just_1/sum(group_counts$just_1, group_counts$just_2, group_counts$just_3, group_counts$gt_3)*100, 1)

unique_count <- sum(group_counts$just_1, group_counts$just_2, group_counts$just_3, group_counts$gt_3)
...
Primary **Cesarean section** rate was **```r prim_c_section_prcnt```%**, constituting **`r prim_c_section_prcnt_all.cs`%** of all c-sections (**```r c_section_prcnt```%**).  Repeat C-Section rate was **```r repeat_cs_prcnt```%**.  
...
There were **```r unique_count```** unique mothers:

- **```r group_counts$just_1```**: one delivery  (**```r just_1_pcnt```%**)

- **```r group_counts$just_2```**: two deliveries

- No more than 2 deliveries for any unique mother during this period _(each pair of twins counts as one delivery)_
...
```
Focusing on first deliveries only, for every 4 delivered pregnancies, one pregnancy was not delivered, giving an overall not-delivered incidence rate of  **```r overall_incidence_rate```%**. At the time of the first deliveries __only__, there is a significant variance by zip code:
...
The top plot shows the forecast graph appended at the end of the historical data, and the following table gives forecasted delivery values. The next graph compares the forecast values with the real delivery count for 2024 Quarter 2. A comparative table follows. Take-away: while there is variation in the accuracy of the forecast compared with the real data, the total deliveries for the second quarter of 2024 is about the same by the forecast (189) or by the real count (187).
